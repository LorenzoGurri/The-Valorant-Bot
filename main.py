...
async def stats(msg, channel, author):
    ...
    if len(msg) == 3:
        ...
        if user != None:
            ...
            await channel.send(
                "**ERROR**: {} not in database\nUse !tvb connect [username#TAG]".format(
                    author
                )
            )
            return
    ...
    response = requests.get(
        api_link + "v1/mmr/{}/{}/{}".format(region, username[0], username[1])
    )
    # Error checking
    if response.status_code != 200:
        await channel.send(
            "**ERROR**: Error with API response\nStatus Code: {}".format(
    ...
    response = requests.get(
        api_link
        + "v3/matches/{}/{}/{}?filter=competitive".format(
            region, player.getUsername(), player.getTag()
        )
    )
    # Error checking
    if response.status_code != 200:
    ...
    output = discord.Embed(
        title="{}'s Statistics".format(username[0]), color=discord.Color.blue()
    )
    output.set_thumbnail(url=player.getImage())
    # Rank and RR
    output.add_field(
        name=player.getRank(), value="{}/100".format(player.getRR()), inline=True
    )
    output.add_field(
        name="RR Change", value="{}".format(player.getRRChange()), inline=False
    )
    # Last 5 Games
    output.add_field(name="K/D", value="{:.2f}".format(player.getKD()), inline=True)
    output.add_field(name="ACS", value="{:.0f}".format(player.getACS()), inline=True)
    output.add_field(
        name="Headshot %", value="{:.1f}%".format(player.getHeadshot()), inline=True
    )
...
async def lineups(msg, channel):
    ...
    if len(msg) != 7:
        await channel.send(
            "**USAGE**: !tvb lineups [map] [site] [type] [start] [agents] "
        )
        return
    Map = msg[2].lower()
    Site = msg[3].lower()
...
async def agents(msg, channel):
    ...
    if len(msg) != 3:
        ...
        output = discord.Embed(
            title="Agent: {} ({})".format(name, role), color=discord.Color.blue()
        )

        output.add_field(name="Description", value=description)
    ...
    if len(msg) != 3:
        ...
        if typing.lower() == "pro":
            ...
            for xhair in data["Crosshairs"]:
                ...
                funHair = MyButton.PlayerButton(
                    xhair["name"], xhair["code"], xhair["image"]
                )
...
async def help(msg, channel, message):
    ...
    msg.add_field(name="Connect", value="!tvb connect [username#TAG]", inline=False)
    msg.add_field(name="Disconnect", value="!tvb disconnect", inline=False)
    msg.add_field(
        name="Stats",
        value="!tvb stats [region] [username#TAG]\n!tvb stats [ap,br,eu,kr,latam,na]",
        inline=False,
    )
    msg.add_field(name="Lineups", value="!tvb lineups", inline=False)
    msg.add_field(name="Agents", value="!tvb agents [Agent Name]", inline=False)
    msg.add_field(name="Feedback", value="!tvb feedback", inline=False)
    msg.add_field(
        name="Help",
